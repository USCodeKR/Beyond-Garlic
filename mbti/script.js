...
function showResult(mbtiOverride) {
    ...
    resultData.relatedProducts.forEach(p => {
        ...
        if (href) {
            a.href = href;
            a.target = "_blank";
            a.rel = "noopener"; // 일부 브라우저에서 noreferrer + _blank가 about:blank 이슈
        } else {
            // 무효 링크: 클릭 비활성 + 스타일 약하게
...
function resolveLink(url) {
    ...
    if (u.startsWith('//')) return 'https:' + u; // 프로토콜 상대 URL
    return 'https://' + u.replace(/^\/+/, '');   // 스킴 없으면 https로 보정
}
